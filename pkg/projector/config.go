...
func getArgs(opts *Opts) ([]string, error) {
    ...
    if operation == Remove {
        if len(opts.Args) != 2 {
            return nil, fmt.Errorf("add requires 1 arguments, but recieved %v", len(opts.Args) - 1)
        }
        return opts.Args[1:], nil
    }

    if len(opts.Args) > 1 {
        return nil, fmt.Errorf("add requires 0 or 1 arguments, but recieved %v", len(opts.Args) - 1)
    }
